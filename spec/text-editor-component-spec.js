...
describe('TextEditorComponent', () => {
  ...
  ffit('resizes based on the content when the autoHeight and/or autoWidth options are true', async () => {
    ...
    console.log('check');
    console.log(gutterContainer.offsetWidth, scrollContainer.scrollWidth);
  ...
  describe('autoscroll on cursor movement', () => {
    it('automatically scrolls vertically when the requested range is within the vertical scroll margin of the top or bottom', async () => {
      ...
      await component.getNextUpdatePromise()
      expect(scrollContainer.scrollTop).toBe((3 - editor.verticalScrollMargin) * component.measurements.lineHeight)
    ...
    it('does not vertically autoscroll by more than half of the visible lines if the editor is shorter than twice the scroll margin', async () => {
      ...
      await component.getNextUpdatePromise()
      expect(scrollContainer.scrollTop).toBe((4 - scrollMarginInLines) * component.measurements.lineHeight)
    ...
    it('automatically scrolls horizontally when the requested range is within the horizontal scroll margin of the right edge of the gutter or right edge of the screen', async () => {
      ...
      expect(scrollContainer.scrollLeft).toBe(expectedScrollLeft)
    ...
    it('does not horizontally autoscroll by more than half of the visible "base-width" characters if the editor is narrower than twice the scroll margin', async () => {
      ...
      const contentWidth = scrollContainer.clientWidth - gutterContainer.offsetWidth
    ...
    it('correctly autoscrolls after inserting a line that exceeds the current content width', async () => {
      ...
      expect(scrollContainer.scrollLeft).toBe(component.getScrollWidth() - scrollContainer.clientWidth)
    })
    ...
    it('accounts for the presence of horizontal scrollbars that appear during the same frame as the autoscroll', async () => {
      ...
      expect(scrollContainer.scrollTop).toBe(component.getScrollHeight() - scrollContainer.clientHeight)
      expect(scrollContainer.scrollLeft).toBe(component.getScrollWidth() - scrollContainer.clientWidth)
    })
  })
  ...
  describe('highlight decorations', () => {
    ...
    it('can flash highlight decorations', async () => {
      ...
      component.refs.scrollContainer.scrollTop = 100
      await component.getNextUpdatePromise()
    ...
    describe('on the lines', () => {
      ...
      it('autoscrolls the content when dragging near the edge of the screen', async () => {
        ...
        function assertScrolledDownAndRight () {
          ...
          expect(scrollContainer.scrollLeft).toBeGreaterThan(previousScrollLeft)
          previousScrollLeft = scrollContainer.scrollLeft
        }

        function assertScrolledUpAndLeft () {
          expect(scrollContainer.scrollTop).toBeLessThan(previousScrollTop)
          previousScrollTop = scrollContainer.scrollTop
          expect(scrollContainer.scrollLeft).toBeLessThan(previousScrollLeft)
          previousScrollLeft = scrollContainer.scrollLeft
        }
    ...
    describe('on the line number gutter', () => {
      ...
      it('autoscrolls the content when dragging near the edge of the screen', async () => {
        ...
        function assertScrolledDown () {
          expect(scrollContainer.scrollTop).toBeGreaterThan(previousScrollTop)
          previousScrollTop = scrollContainer.scrollTop
          expect(scrollContainer.scrollLeft).toBe(previousScrollLeft)
          previousScrollLeft = scrollContainer.scrollLeft
        }
